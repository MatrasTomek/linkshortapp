...
import { z } from 'zod';
import { createShortLink, deleteShortLink, updateShortLink } from '@/data/short-links';
...
export async function createShortLinkAction(input: CreateShortLinkActionInput): Promise<CreateShortLinkActionResult> {
	...
	if (!parsedInput.success) {
		const firstError = parsedInput.error.issues.at(0)?.message ?? 'Niepoprawne dane formularza.';
	...
	if (!result.success) {
		...
		if (result.error === 'SHORT_CODE_GENERATION_FAILED') {
			return { success: false, error: 'Nie udało się wygenerować unikalnego kodu. Spróbuj ponownie.' };
		}
...
export async function updateShortLinkAction(input: UpdateShortLinkActionInput): Promise<UpdateShortLinkActionResult> {
	const { userId } = await auth();
...
export async function deleteShortLinkAction(input: DeleteShortLinkActionInput): Promise<DeleteShortLinkActionResult> {
	const { userId } = await auth();
